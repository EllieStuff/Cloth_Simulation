...
public:
	...
	Mesh(int _width, int _height) {
		...
		for (int row = 0; row < width; row++) {
			...
			for (int col = 0; col < height; col++) {
				...
				particles[idx].pos = glm::vec3(row * 0.3f, col * 0.2f, 0.0f);
	...
	void Mesh::UpdateSpeed(float dt)
	{
		for (int i = 0; i < springs.size(); i++) {
			...
			//glm::vec3 force = -(currSpring.K_ELASTICITY * (p2p1Vector.length() - currSpring.REST_DIST)
			//	+ glm::dot(currSpring.K_DAMPING /* * (v1 - v2) */, p2p1NormalizedVector)) * p2p1NormalizedVector;
